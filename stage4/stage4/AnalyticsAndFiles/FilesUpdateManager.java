package stage4.AnalyticsAndFiles;

import stage4.FoodStaff;
import stage4.Staff;
import stage4.FoodRelated.Food;
import stage4.FoodRelated.Item;
import stage4.OrdersAndPayment.DetailFoodBought;
import stage4.OrdersAndPayment.DetailTicketBought;
import stage4.TicketRelated.Movie;
import stage4.TicketRelated.Screen;
import stage4.TicketRelated.SeatingArrangement;
import stage4.TicketRelated.Showtime;
import stage4.util.Path;

import java.io.BufferedWriter;
import java.io.FileWriter;
import java.io.IOException;
import java.time.LocalDateTime;
import java.time.ZoneId;
import java.time.temporal.ChronoUnit;
import java.util.ArrayList;
import java.util.Map;
import java.util.TreeMap;

public class FilesUpdateManager {

   // Initializes Files Update Manager
   private FilesUpdateManager() {}

    /**
     * Updates the ticket sales file with the current order's details (for report)
     */
    public static void updateTicketSalesFile(String paymentId, DetailTicketBought detail) {

        String movieTitle = detail.getShowtime().getMovie().getTitle();

        byte screenId = detail.getShowtime().getScreen().getScreenID();

        String startTime = detail.getShowtime().getStartTime().toString();

        ArrayList<byte[]> chosenSeats = detail.getChosenSeats();

        String screenType = detail.getShowtime().getScreen().getScreenType();

        byte quantity = (byte) (detail.getImaxNum() + detail.getNormalNum());

        short paymentAmount = detail.getPaymentAmount();

        // Formats file
        String line = "";

        line += paymentId;
        line += ",";
        line += LocalDateTime.now(ZoneId.of("US/Mountain")).truncatedTo(ChronoUnit.MINUTES).toString();
        line += ",";
        line += paymentAmount;
        line += ",";
        line += quantity;
        line += ",";
        line += screenType;
        line += ",";
        line += screenId;
        line += ",";
        line += movieTitle;
        line += ",";
        line += startTime;
        line += ";";

        // Selected seats
        for (byte[] aSeat : chosenSeats) {
            line += aSeat[0];
            line += ",";
            line += aSeat[1];
            line += ",";
        }

        // append to a new line in the ticket sales report file
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(Path.TICKET_SALES_REPORT_PATH, true))) {
            writer.write(line);
            writer.newLine();
        } catch (IOException e) {
            System.err.println(e.getMessage());
        }
    }

    /**
     * Updates the food sales file with the current order's details (for report)
     */
    public static void updateFoodSalesFile(String paymentId, DetailFoodBought detail) {

        short paymentAmount = detail.getPaymentAmount();

        TreeMap<Food, Byte> orderedFood = detail.getOrderedFood();
        /*
         * adds the information about the transaction in the following format:
         * LocalDateTime object, stage4.TicketRelated.stage4.Orders.Payment amount (total price)
         * then separate these two with ";"
         * after that, append in an alternating way, separated by ","
         * the food name, the quantity, and the total price of one food menu
         */

        String line = "";
        line += paymentId;
        line += ",";
        line +=  LocalDateTime.now(ZoneId.of("US/Mountain")).truncatedTo(ChronoUnit.MINUTES).toString();
        line += ",";
        line += paymentAmount;
        line += ";";

        for (Food aFood : orderedFood.keySet()) {
            line += aFood.getMenuName();
            line += ",";
            line += String.valueOf(orderedFood.get(aFood));
            line += ",";
            line += String.valueOf(aFood.getPrice() * orderedFood.get(aFood));
            line += ",";
        }

        // append to a new line in the food sales report file
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(Path.FOOD_SALES_REPORT_PATH, true))) {
            writer.write(line);
            writer.newLine();
        } catch (IOException e) {
            System.err.println(e.getMessage());
        }
    }

    /**
     * Updates the inventory file with the current inventory state.
     * Writes the item name and its remaining quantity to the file (for report + data persistence)
     */
    public static void updateInventoryFile(ArrayList<Item> itemList) {

        // store all the data in an array of Strings
        String[] lines = new String[itemList.size()];

        for (int i = 0; i < itemList.size(); i++) {
            Item item = itemList.get(i);
            if (item.hasItemName() && item.hasQuantity() && item.hasItemId() && item.hasBuyingCost()) {
                lines[i] = item.getItemId() + ","+ item.getItemName() + "," + item.getQuantity() + "," + item.getBuyingCost();
            }
        }

        // overwrites the previous data with the lines of String already prepared
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(Path.INVENTORY_REPORT_PATH))) {
            for (String aLine : lines) {
                writer.write(aLine);
                writer.newLine();
            }
        } catch (IOException e) {
            System.err.println(e.getMessage());
        }
    }

    /**
     * Updates the item order file with the details of the ordered item, including the
     * item name, quantity ordered, and total price (for report)
     * @param item The item being ordered.
     * @param buyingQuantity The quantity of the item being ordered.
     * @param totalPrice The total price for the ordered quantity.
     */
    public static void updateItemOrderFile(Item item, short buyingQuantity, short totalPrice) {

        LocalDateTime dateTime = LocalDateTime.now(ZoneId.of("US/Mountain")).truncatedTo(ChronoUnit.MINUTES);

        String line = "";

        line += (dateTime + "," + item.getItemName() + "," + buyingQuantity + "," + totalPrice);

        /*
         * stores the item order receipt into a file with the following format:
         * LocalDateTime, item name, quantity bought, and the total price
         */
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(Path.ITEM_ORDER_REPORT_PATH, true))) {
            writer.write(line);
            writer.newLine();
        } catch (IOException e) {
            System.err.println(e.getMessage());
        }
    }

    /**
     * updates the current food data available in the file (data persistence)
     * @param menuList
     */
    public static void updateFoodDataFile(ArrayList<Food> menuList) {
        // store all data in a list of String
        String[] lines = new String[menuList.size()];

        for (int i=0; i<menuList.size(); i++) {
            Food food = menuList.get(i);

            Byte foodId = food.getMenuId();
            String foodName = food.getMenuName();
            Byte foodPrice = food.getPrice();

            lines[i] = foodId + "," + foodName + "," + foodPrice + ";";

            TreeMap<Item, Byte> recipeList = food.getRecipe();

            for (Map.Entry<Item, Byte> aRecipe : recipeList.entrySet()) {
                Item item = aRecipe.getKey();
                byte itemId = item.getItemId();
                byte qty = aRecipe.getValue();
                lines[i] = lines[i] + itemId + "/" + qty + ",";
            }

        }

        try (BufferedWriter writer = new BufferedWriter(new FileWriter(Path.FOOD_DATA_PATH))) {
            for (String aLine : lines) {
                writer.write(aLine);
                writer.newLine();
            }
        } catch (IOException e) {
            System.err.println(e.getMessage());
        }
    }

    public static void updateStaffDataFile(ArrayList<Staff> staffs) {
        // store all data in a list of string
        String[] lines = new String[staffs.size()];

        for (int i=0; i< staffs.size(); i++) {
            Staff staff = staffs.get(i);

            String staffName = staff.getEmployeeName();
            byte staffId = staff.getEmployeeId();
            byte staffHourlyRate = staff.getHourlyRate();
            String staffSchedule = staff.getSchedule();

            lines[i] = staffName + "," + staffId + "," + staffHourlyRate + "," + staffSchedule + ",";

            if (staff instanceof FoodStaff) {
                lines[i] += "food";
            } else {
                lines[i] += "ticket";
            }
        }

        try (BufferedWriter writer = new BufferedWriter(new FileWriter(Path.STAFF_DATA_PATH))) {
            for (String aLine : lines) {
                writer.write(aLine);
                writer.newLine();
            }
        } catch (IOException e) {
            System.err.println(e.getMessage());
        }

    }


    public static void updateMovieDataFile(ArrayList<Movie> movies) {
        // store all movie data in a list
        String[] lines = new String[movies.size()];

        for (int i=0; i<movies.size(); i++) {
            Movie movie = movies.get(i);

            byte movieId = movie.getMovieID();
            String movieName = movie.getTitle();
            String movieGenre = movie.getGenre();
            short movieDuration = movie.getDurationMinutes();

            lines[i] = movieId + "," + movieName + "," + movieGenre + "," + movieDuration;
        }

        try (BufferedWriter writer = new BufferedWriter(new FileWriter(Path.MOVIE_DATA_PATH))) {
            for (String aLine : lines) {
                writer.write(aLine);
                writer.newLine();
            }
        } catch (IOException e) {
            System.err.println(e.getMessage());
        }

    }

    public static void updateScreenDataFile(ArrayList<Screen> screens) {
        // store all screen data in a list
        String[] lines = new String[screens.size()];

        for (int i=0; i<screens.size(); i++) {
            Screen screen = screens.get(i);

            byte screenId = screen.getScreenID();
            String screenType = screen.getScreenType();

            lines[i] = screenId + "," + screenType;
        }

        try (BufferedWriter writer = new BufferedWriter(new FileWriter(Path.SCREEN_DATA_PATH))) {
            for (String aLine : lines) {
                writer.write(aLine);
                writer.newLine();
            }
        } catch (IOException e) {
            System.err.println(e.getMessage());
        }
    }

    public static void updateShowtimeDataFile(ArrayList<Showtime> showtimes) {
        // store all showtime data in a list
        String[] lines = new String[showtimes.size()];

        for (int i=0; i< showtimes.size(); i++) {
            Showtime showtime = showtimes.get(i);

            byte showtimeId = showtime.getShowtimeID();
            byte movieId = showtime.getMovie().getMovieID();
            byte screenId = showtime.getScreen().getScreenID();
            byte hour = (byte) showtime.getStartTime().getHour();
            byte minute = (byte) showtime.getStartTime().getMinute();

            lines[i] = showtimeId + "," + movieId + "," + screenId + "," + hour + "," + minute + ";";

            SeatingArrangement seatings = showtime.getScreen().getSeating();

            int rows = seatings.getRowCapacity();
            int cols = seatings.getColCapacity();

            for (int j=0; j<rows; j++) {
                for (int k=0; k<cols; k++) {
                    if (seatings.getSeatStatus(j, k)) {
                        lines[i] += "1,";
                    } else {
                        lines[i] += "0,";
                    }
                }
                lines[i] += "/";
            }
        }

        try (BufferedWriter writer = new BufferedWriter(new FileWriter(Path.SHOWTIME_DATA_PATH))) {
            for (String aLine : lines) {
                writer.write(aLine);
                writer.newLine();
            }
        } catch (IOException e) {
            System.err.println(e.getMessage());
        }

    }

}
